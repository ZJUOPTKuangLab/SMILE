...
function [lpx, lpy, para_est, result_xy, result_6N, result_init, final_x, final_y, final_z, N_x, N_y, N_z, bg_x, bg_y, bg_z, frame] = process_round_data_ModLoc_XY(tmp_image, coeff, phase_difference, x_pixelsize, z_pixelsize, boxsz, thresh_dist, thresh_low, thresh_high, ...
...
%     rangez = 300;
    ...
    %% 20260128 利用modloc的方法进行处理(X和Y方向同时处理，并且进行平均) ==================================================================================================================================
    ...
    k_num1 = round((result_p1- p1)/2/pi);
    k_num2 = round((result_p2- p2)/2/pi);
    
    T1 = 2*pi/lpx(2)*x_pixelsize;
    T2 = 2*pi/lpy(2)*x_pixelsize;
    
    p1_vector = [cos(theta_1),sin(theta_1)];
    p2_vector = [cos(theta_2),sin(theta_2)];
    
    rc_1 = p1_vector(1)*final_x + p1_vector(2)*final_y;
    rc_2 = p2_vector(1)*final_x + p2_vector(2)*final_y;
    
    
    fine_z1 = (k_num1 + p1/2/pi - phase_1/2/pi)*T1/sin(thetaz_1)-rc_1/tan(thetaz_1);
    fine_z2 = (k_num2 + p2/2/pi - phase_2/2/pi)*T2/sin(thetaz_2)-rc_2/tan(thetaz_2);
    
    final_z = (fine_z1 + fine_z2)/2;
