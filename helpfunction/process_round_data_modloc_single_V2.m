...
function [lpx, lpy, para_est, result_xy, result_6N, result_init, final_x, final_y, final_z, N_x, N_y, N_z, bg_x, bg_y, bg_z, frame] = process_round_data_modloc_single_V2(tmp_image, coeff, phase_difference, x_pixelsize, z_pixelsize, boxsz, thresh_dist, thresh_low, thresh_high, ...
...
%     rangez = 300;
    ...
    %% 20260120 用6N的结果来进行ROSE的处理(只针对Y方向进行处理) ==================================================================================================================================
    N_x = tx*x_pixelsize;
    N_y = ty*x_pixelsize;
    N_z = tz*z_pixelsize;
    ...
    thetaz_2 = lpy(3);
    phase_2 = lpy(4);
    result_p2 = (tx.*cos(theta_2).*cos(thetaz_2) + ty.*sin(theta_2).*cos(thetaz_2)+tz_modify .*sin(thetaz_2)) * cycle_2 + phase_2;
    
    dp1 = checkPhase(result_p1 - p1);
    dp2 = checkPhase(result_p2 - p2);
    
    T1 = 2*pi/lpx(2)*x_pixelsize;
    T2 = 2*pi/lpy(2)*x_pixelsize;
    x_vector = [1, 0, 0];
    y_vector = [0, 1, 0];
    z_vector = [0, 0, 1];
    p1_vector = [cos(theta_1).*cos(thetaz_1),sin(theta_1).*cos(thetaz_1), sin(thetaz_1)];
    p2_vector = [cos(theta_2).*cos(thetaz_2),sin(theta_2).*cos(thetaz_2), sin(thetaz_2)];
    
    % calculate the relative projection on each dimension
    dx_p1 = dp1*sum((x_vector.*p1_vector))/2/pi*T1;  % unit: nm
    dy_p1 = dp1*sum((y_vector.*p1_vector))/2/pi*T1;
    dz_p1 = dp1*sum((z_vector.*p1_vector))/2/pi*T1;
    
    dx_p2 = dp2*sum((x_vector.*p2_vector))/2/pi*T2;
    dy_p2 = dp2*sum((y_vector.*p2_vector))/2/pi*T2;
    dz_p2 = dp2*sum((z_vector.*p2_vector))/2/pi*T2;
    
    % 这里只使用dz_p
    fine_z = N_z - (dz_p1+dz_p2)/2;
    ...
    % final_z = (result_xy(:,7) - size(coeff,3)/2)*z_pixelsize;  % ====================== 20260120 修正===================================================================================================
    final_z = fine_z;  % ====================== 20260120 修正===================================================================================================
