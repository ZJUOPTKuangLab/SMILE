...
function [lpx, lpy, para_est, result_xy, result_6N, result_init, final_x, final_y, final_z, N_x, N_y, N_z, bg_x, bg_y, bg_z, frame] = process_round_data_nanogrid_v1_norolling(tmp_image, coeff, phase_difference, x_pixelsize, z_pixelsize, boxsz, thresh_dist, thresh_low, thresh_high, ...
    Nbound, uplimx13, lowlimx13, uplimx2, lowlimx2,...
    uplimy13, lowlimy13, uplimy2, lowlimy2, rangez, rangez_center, lpx_last_round, lpy_last_round)

    %% 1. Pre-process
    
    image_stack = zeros(size(tmp_image,1), size(tmp_image,2), size(tmp_image,3)/6, 6);
    image_stack(:,:,:,1) = tmp_image(:,:,1:6:end);
    image_stack(:,:,:,2) = tmp_image(:,:,2:6:end);
    image_stack(:,:,:,3) = tmp_image(:,:,3:6:end);
    image_stack(:,:,:,4) = tmp_image(:,:,4:6:end);
    image_stack(:,:,:,5) = tmp_image(:,:,5:6:end);
    image_stack(:,:,:,6) = tmp_image(:,:,6:6:end);
    clear tmp_image
...
%     imageslicer(single(a));
    ...
    %% 9. 进一步refine 最后的结果
%     rangez = 300;
    mask_z = tz> (rangez_center-rangez)/z_pixelsize & tz< (rangez_center+rangez)/z_pixelsize;
    tx1 = tx(mask_z);
    ty1 = ty(mask_z);
    tz1 = tz(mask_z);
...
%             [lpx_last_round(1) lpx_last_round(2) lpx_last_round(3) lpx_last_round(4)], 0); 
      [lpx, ~] = FitPhasePlane3D_nanogrid_v1(tx1, ty1, tz1/x_pixelsize*z_pixelsize, p1(mask_z), ...
        [lpx_last_round(1) lpx_last_round(2) lpx_last_round(3) lpx_last_round(4)], 0);  % nano grid upgrade v1 ========================================================================================
...
%             [lpy_last_round(1) lpy_last_round(2) lpy_last_round(3) lpy_last_round(4)], 0); 
    [lpy, ~] = FitPhasePlane3D_nanogrid_v1(tx1, ty1, tz1/x_pixelsize*z_pixelsize, p2(mask_z), ...
        [lpy_last_round(1) lpy_last_round(2) lpy_last_round(3) lpy_last_round(4)], 0); % nano grid upgrade v1 ========================================================================================
